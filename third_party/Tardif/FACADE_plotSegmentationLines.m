function [mClr] = FACADE_plotSegmentation(im,  vsEdges,  vClass, vP_v, vbOutliers, algoNum, noIm, f, points)
  ...
  %      imshow(im);
    ...
    for c=nbClass:-1:1        
        ...
        for i=1:length(vsEdges_s)
            ...
            middlePoint(1) = vPts(1,1);middlePoint(2) = vPts(2,1);middlePoint(3) = 1;
            %if (c > 1 && c < 4)
            %point(1) = points (c, 1); 
            %point(2) = points (c, 2);
            %else
            ...
            if (abs (point(1) - middlePoint(1)) > abs(point(2) - middlePoint(2)))
             ...
             %if (f == 0)
             %   mClr(4,:) = [0 0 0];
            % 	plot(x, y, 'Color',  mClr(c,:), 'LineWidth', 3 );
            % else
                 plot(x, y, 'Color',  mClr(c,:), 'LineWidth', 3 );
            % end
        end
    ...
    if only3
        ...
        if ~isempty(vPts)            
            plot(vPts(1,1:STEP:end), vPts(2,1:STEP:end),MARKER, 'Color', [COL,COL,COL], 'LineWidth', 1  );
        end
        hold off
    end
    ...
    if  ~isempty(vbOutliers) && sum(vbOutliers)>=1
        ...
        plot(vPts(1,1:STEP:end), vPts(2,1:STEP:end),'.', 'Color', [0,0,0], 'LineWidth', 1 );
    end    
    hold off
